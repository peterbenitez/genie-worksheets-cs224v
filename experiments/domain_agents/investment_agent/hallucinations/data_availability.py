...
class DataAvailabilityChecker:
    ...
    of hard constraints (schema) before invoking expensive LLM analysis.
    ...
    def check_availability(self, user_utterance: str, query_context: Dict = None) -> DataAvailabilityResult:
        ...
        # Phase 1: Check missing columns (fast, deterministic)
        ...
        # Phase 2: Check missing tables (fast, deterministic)
    ...
    def _identify_target_table(self, utterance: str) -> Optional[str]:
        ...
        Keywords are auto-generated from table names (e.g., fidelity_funds -> fund, funds).
        """
...
def test_data_availability_checker():
    ...
    # Test 1: Missing column (bond query asking for fund-specific field)
    result1 = checker.check_availability("What's the expense ratio for bond CUSIP 12345?")
    print(f"\nTest 1 (Missing Column): {result1.issue_type}")
    print(f"  Explanation: {result1.explanation}\n")

    # Test 2: Valid query
    result2 = checker.check_availability("What are the returns for FXAIX?")
    print(f"Test 2 (Valid Query): {result2.issue_type}")
    print(f"  Explanation: {result2.explanation or 'No issue detected'}\n")
