...
MAX_TOOLS = 5  # Hard limit - forces consolidation to exactly this many tools
...
class RewrittenContext:
    ...
    user_profile: str  # Generic user profile field
...
{{
  "tools": {{
    "tool_name_1": {{
      ...
      "original": "Dict[str, Any] (performance metrics for each entity)"
    }},
    "tool_name_2": {{
      "python_type": "str",
...
class ToolClusterer:
    ...
    def __init__(self, return_type_analyzer: ReturnTypeAnalyzer, client: AzureOpenAI, model: str = DEFAULT_MODEL):
    ...
    def _extract_action(self, tool_name: str, description: str) -> str:
        """Extract primary action verb from tool name or description using LLM"""
...
- Entity subtypes (type_a vs type_b vs type_c -> all "entities")
...
class SemanticMappingRewriter:
    ...
    def rewrite_context(
        ...
        return RewrittenContext(
            user_utterance=context.get("user_utterance", context.get("query", "")),
            timestamp=context.get("timestamp", ""),
            hallucinated_response=context.get("hallucinated_response", ""),
            turn_number=context.get("turn_number", 0),
            allow_hallucination=context.get("allow_hallucination", True),
...
USER UTTERANCE:
"{context.get('user_utterance', context.get('query', ''))}"
...
class BroadToolDetector:
    ...
    def detect_broad_tools(self, tools: List[HypotheticalTool]) -> List[Tuple[str, str]]:
        """
        Detect tools that are overly broad using LLM.
        ...
        for tool in tools:
            prompt = f"""Analyze if this tool is overly broad (tries to do too many different things).

Tool name: {tool.tool_name}
Description: {tool.description}
Parameters: {[p.name for p in tool.parameters]}

Is this tool overly broad? Return JSON:
{{"is_broad": true/false, "reason": "brief reason if broad"}}"""
            ...
            try:
                ...
                if result.get("is_broad"):
                    broad_tools.append((tool.tool_name, result.get("reason", "Overly broad")))
            except Exception:
                pass
...
class ToolRegistryRefiner:
    ...
    def __init__(self, model: str = DEFAULT_MODEL):
        ...
        self.tool_clusterer = ToolClusterer(self.return_type_analyzer, self.client, model)
        self.merge_analyzer = MergeAnalyzer(self.client, model)
        self.semantic_rewriter = SemanticMappingRewriter(self.client, model)
        self.broad_tool_detector = BroadToolDetector(self.client, model)
    ...
    def refine_registry(self, registry: ToolRegistry) -> ToolRegistry:
        ...
        # Step 4: Force consolidation to MAX_TOOLS if still over limit
        if len(refined_tools) > MAX_TOOLS:
            logger.info(f"Still have {len(refined_tools)} tools, consolidating to {MAX_TOOLS}...")
            refined_tools = self._consolidate_to_max_tools(refined_tools)

        # Step 5: Detect broad tools
        broad_tools = self.broad_tool_detector.detect_broad_tools(list(refined_tools.values()))
        if broad_tools:
        ...
        # Step 6: Create refined registry
    ...
    def _create_merged_tool(
        ...
        for tool_name in decision.tool_names_to_merge:
            ...
            for context in old_tool.conversation_contexts:
                ...
                # Convert back to dict (use 'query' for consistency with existing format)
                context_dict = {
                    "query": rewritten.user_utterance,
                    ...
                    "user_profile": rewritten.user_profile,
    ...
    def _consolidate_to_max_tools(self, tools: Dict[str, HypotheticalTool]) -> Dict[str, HypotheticalTool]:
        """Use LLM to consolidate tools down to MAX_TOOLS limit."""
        ...
        prompt = f"""You must consolidate {len(tools)} tools into EXACTLY {MAX_TOOLS} generalized tools.

CURRENT TOOLS:
{chr(10).join(tool_summaries)}

CONSOLIDATION RULES:
1. Group tools by their PRIMARY ACTION (the main verb they perform)
2. Each consolidated tool should be parameterized to handle all its grouped tools
3. Use generic names following the pattern "action_subject" (e.g., "action_entity", "action_items")
4. Merge tools that do similar things even if on different entities
...
{{
    "consolidated_tools": [
        {{
            "name": "tool_name",
            "description": "what it does (covers all grouped tools)",
            "parameters": [{{"name": "param", "type": "str", "required": true}}],
            "consolidates": ["original_tool1", "original_tool2", ...]
        }}
    ]
}}

IMPORTANT: Return EXACTLY {MAX_TOOLS} tools. No more, no less."""
        ...
        try:
            ...
            result = json.loads(content)
            consolidated = result.get("consolidated_tools", [])

            # Build new tools dict
            new_tools = {}
            for cluster in consolidated:
                tool_name = cluster["name"]
                consolidates = cluster.get("consolidates", [])

                # Calculate total frequency
                frequency = sum(tools[t].frequency for t in consolidates if t in tools)

                # Collect all contexts
                all_contexts = []
                for t in consolidates:
                    if t in tools:
                        all_contexts.extend(tools[t].conversation_contexts)
                ...
                params = [
                    ...
                    ) for p in cluster.get("parameters", [])
                ]

                new_tools[tool_name] = HypotheticalTool(
                    tool_name=tool_name,
                ...
                logger.info(f"  - {tool_name}: {frequency} instances (from {len(consolidates)} tools)")
...
def main():
    ...
    # Show refined tools by action (simple extraction from tool name)
    action_counts = defaultdict(int)
    for tool in refined_registry.missing_tools.values():
        # Extract action from tool name (first part before underscore)
        action = tool.tool_name.split('_')[0] if '_' in tool.tool_name else tool.tool_name
