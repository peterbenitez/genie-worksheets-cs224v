...
def get_llm_client(*, model: str, slow_rate_limiter: bool = False, **kwargs):
    ...
    if "azure/" in model:
        ...
        # ChatGoogleGenerativeAI expects GOOGLE_API_KEY env var or api_key parameter
        api_key = os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY")
        return GeminiModelCall(
            model=model.split("/")[-1],
            api_key=api_key,
...
class GeminiModelCall(ChatGoogleGenerativeAI):
    """Gemini model wrapper using LangChain's official Google Generative AI integration."""
    ...
    async def agenerate(
        ...
        **kwargs: Any,
        ...
        if response_format:
            ...
            response_format = json.dumps(response_format)
        cache_key = generate_cache_key(serialize_messages, self.model, response_format)
